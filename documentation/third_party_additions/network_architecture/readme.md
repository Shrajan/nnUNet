...
4. The various architecture-specific ***planners*** can be found in `nnunetv2/experiment_planning/experiment_planners/`. For example, the planner for `Attention U-Net` is called as **nnUNetPlannerAttUNet** inside the ***attentionUNet_planner.py*** file, and the plans file are saved as **nnUNetPlansAttUNet**.
